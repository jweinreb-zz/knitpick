...
RAVELRY_USERNAME='4CEB364E4FD3892C1EC0'
RAVELRY_PASSWORD='61LaiyoHLxlHSCrL69cRQNe4EPtxvaBOEvggWYRJ'
...
patt_attrs = requests.get('https://api.ravelry.com/pattern_attributes/groups.json', auth=auth)
...
## Pattern Categories
patt_cats = requests.get('https://api.ravelry.com/pattern_categories/list.json', auth=auth)
...
for cat in accessories_and_clothing:
    ...
    if children:
        for child in children:
            ...
            if childdren:
                for childd in childdren:
                    record =  {k: v for k, v in childd.items() if k in ['id', 'long_name']}
                    records.append(record)        	
