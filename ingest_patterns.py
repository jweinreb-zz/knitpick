# Knitpick: Find Knitwear Patterns You'll Love

# https://medium.com/data-from-the-trenches/recommendation-engines-101-8fcb6498796

# https://www.ravelry.com/statistics/users

# https://github.com/jtrudell/ravelry-api-wrapper

#https://pdfs.semanticscholar.org/4021/3f7d7a6b6d97fa60830d30cd0e882c2e21c1.pdf
...
def clean_attribute_text(attribute: str):
    ...
    att2 = att1.replace(" - ", "_").replace('-', '_').replace(" / ", "_").replace("/", "_")
...
PATTERN_ATTRIBUTES.loc[:, 'name'] = PATTERN_ATTRIBUTES.name.apply(lambda x: clean_attribute_text(x))
PATTERN_ATTRIBUTES = PATTERN_ATTRIBUTES.drop_duplicates(subset='name')

NEEDLES = pd.read_csv("needle_sizes.csv")
NEEDLES.loc[:, 'us'] = NEEDLES.us.apply(lambda x: str(x).replace('/', 's').replace('.5', 'h') if
    ('/' in str(x)) or ('.5' in str(x)) else str(x.split('.')[0]))
...
SEARCH_PATTERNS_URL = ENDPOINT + '/patterns/search.json?craft=knitting&pc=%s'  # Get patterns for a category
SEARCH_PATTERNS_URL_PAGE = ENDPOINT + '/patterns/search.json?craft=knitting&pc=%s&page=%d' # Get specific page # of pattern search results
LIST_PATTERN_PROJECTS_URL = ENDPOINT + '/patterns/%d/projects.json'  # Get projects for a pattern
SINGLE_PATTERN_DETAIL_URL = ENDPOINT + '/patterns/%d.json'  # Get details for a single pattern
MULTIPLE_PATTERN_DETAIL_URL = ENDPOINT + '/patterns.json?ids=%s'  # ints separated by +, i.e. 600+818
LIST_USER_PROJECTS_URL = ENDPOINT + '/projects/%s/list.json'  # Get a user's project list
...
## Get Pattern Data
categories = ['pullover', 'cardigan', 'other-sweater','coat', 'dress', 'shrug', 'skirt', 'vest',
              'tee', 'sleeveless-top', 'strapless-top', 'other-top',
              'ankle', 'knee-highs', 'mid-calf', 'thigh-high', 'toeless', 'tube', 'other-socks',
              'scarf', 'cowl', 'shawl-wrap', 'fingerless', 'gloves', 'mittens', 'hat']


orig_patt_keys = ['published', 'name', 'permalink', 'url',
                  'difficulty_average', 'difficulty_count', 'rating_average', 'comments_count',
                  'rating_count', 'favorites_count', 'projects_count', 'queued_projects_count',
                  'free', 'price', 'currency', 'generally_available', 'ravelry_download', 'downloadable',
                  'yardage_max']
...
def parse_pattern(pattern_data:dict)->dict or None:
    ...
    patt_info['author_pattern_count'] = pattern_data['pattern_author']['knitting_pattern_count']
    patt_info['author_favorites_count'] = pattern_data['pattern_author']['favorites_count']
    ...
    # row gauge
    gauge_divisor = pattern_data['gauge_divisor'] if pattern_data['gauge_divisor'] else 4
...
def get_pattern_records_in_category(category:str)->list:
    ''' Return list of records, one for each pattern in specified category '''
    # Number of pages and number of results for each category of pattern
    paginator = requests.get(SEARCH_PATTERNS_URL % category, auth=auth).json()['paginator']
    n_pages, n_patterns = paginator['page_count'], paginator['results']
    logger.info('Found %d %s patterns, %d pages total' % (n_patterns, category, n_pages))
    ...
    for i in range(n_pages):
        ...
        resp = requests.get(SEARCH_PATTERNS_URL_PAGE % (category, i+1), auth=auth).json()
...
def get_user_ids_by_pattern(pattern_id: int):
    ...
    for i in range(n_pages):
        logger.info('Getting page %d of %d' % (i+1, n_pages+1))
        if i > 0:
            resp = requests.get((ingest_patterns.LIST_PATTERN_PROJECTS_URL % pattern_id) + ('/?page=%s' % str(i+1)),
...
def insert_pattern_info_in_db(patt_info: dict, conn_dict: dict=CONN_DICT,
    db_name: str='insight', table_name: str='patterns'):
    with psycopg2.connect(**CONN_DICT) as conn, conn.cursor() as cur:
        query = "INSERT INTO patterns3({}) VALUES ({});".format(", ".join(list(patt_info.keys())),
            ...
            for j, (patt_id, patt_data) in enumerate(patterns.items()):
                ...
                if patt_info is not None:
                    ...
                    try:
                        ...
                        logger.error(f'Pattern {patt_id} does not have US needles; skipping')
...
table_query = table_query_base + ', '.join([x + ' boolean' for x in needles_cols + features_cols]) + ');'


if __name__ == "__main__":
    categories = ['cardigan', 'other-sweater','coat', 'dress', 'shrug', 'skirt', 'vest']
